...
interface ITransactionReportTableProps<TData extends ITransactionReport, TValue> {
...
const globalFilterFn = (row: { original: ITransactionReport }, columnId: string, value: string) => {
  const searchValue = value.toLowerCase();
  const id = row.original.id?.toLowerCase() || '';
  const firstName = row.original.cashier?.firstName?.toLowerCase() || '';
  const lastName = row.original.cashier?.lastName?.toLowerCase() || '';

  return id.includes(searchValue) || firstName.includes(searchValue) || lastName.includes(searchValue);
};

export function DailyReportsDataTable<TData extends ITransactionReport, TValue>({
  columns,
  data,
  ...
  const reportsTable = useReactTable({
    ...
    getPaginationRowModel: filteringType === 'frontend' ? getPaginationRowModel() : undefined,
