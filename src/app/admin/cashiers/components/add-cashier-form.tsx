...
import { EGender, EShiftSession } from '@/types/cashier.type';
...
const addCashierFormSchema = z.object({
...
export function AddCashierForm({ onSuccess, onImageUploadSuccess, initialImageData, onRefresh }: AddCashierFormProps) {
  ...
  const form = useForm<z.infer<typeof addCashierFormSchema>>({
    resolver: zodResolver(addCashierFormSchema),
    defaultValues: {
      firstName: '',
      lastName: '',
      email: '',
  ...
  const formFields = [
    ...
    { name: 'password' as const, label: 'Password', placeholder: 'Enter Password', type: 'password' },
  ...
  async function onSubmit(values: z.infer<typeof addCashierFormSchema>) {
    try {
      const submitData = {
        ...values,
      ...
      const response = await apiInstance.post('/cashier/create', submitData);
      onRefresh?.()
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-4'>
        ...
        <Button type='submit'>Add New Cashier</Button>
      </form>
    </Form>
  );
}
